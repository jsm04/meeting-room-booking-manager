...
export class UserRepository extends BaseRepository {
	...
	add(user: UserWithID) {
		return this.db.run(`INSERT INTO users (id, name, email) VALUES (?, ?, ?)`, [user.id, user.name, user.email])
	}
	...
	update(user: UserWithID): void {
		this.db.run(`UPDATE users SET name = ?, email = ? WHERE id = ?`, [user.name, user.email, user.id])
	}
